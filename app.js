...
(function () {
    ...
    function setActiveNav(filter) {
        ...
        const titles = { all: 'All Tasks', today: 'Today', important: 'Important', completed: 'Completed' };
    ...
    function setActiveCategory(category) {
        ...
        pageTitleEl.textContent = category.charAt(0).toUpperCase() + category.slice(1);
        renderTasks();
    }
    ...
    async function requestNotifPermission() {
        if (!('Notification' in window)) {
            showNotifToast('Notifications not supported', 'Your browser doesn\'t support notifications.');
            return false;
        }

        if (Notification.permission === 'granted') return true;
        if (Notification.permission === 'denied') {
            showNotifToast('Notifications blocked', 'Please allow notifications in your browser settings.');
            return false;
        }
    ...
    function sendNightlyNotification() {
        ...
        const todayTasks = tasks.filter(t => t.dueDate === getTodayString() && !t.completed);
        ...
        if (pendingTasks.length === 0) {
            ...
            if (importantTasks.length > 0) parts.push(`${importantTasks.length} important`);
        ...
        // In-app toast
        const msg = pendingTasks.length === 0
    ...
    notifEnabled.addEventListener('change', async () => {
        if (notifEnabled.checked) {
            ...
            saveNotifSettings();
            startNotifScheduler();
            showNotifToast('Notifications enabled', `You'll get reminders at ${formatTime12h(getNotifSettings().time)}`);
        } else {
    ...
    notifTimeInput.addEventListener('change', () => {
        ...
        if (notifSettings.enabled) {
            showNotifToast('Reminder updated', `Notifications set for ${formatTime12h(notifTimeInput.value)}`);
        }
    });
            ...
            navigator.serviceWorker.register('./sw.js', { scope: './' })
                .then((registration) => {
                    ...
                    if ('periodicSync' in registration) {
                        registration.periodicSync.register('flowdo-nightly-check', {
